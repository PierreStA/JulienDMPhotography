...
const findOne = async (id) => {
  try {
    const [cart_product] = await db.query("SELECT cp.*, p.description, p.price, p.photo FROM `cart_product` cp INNER JOIN `product` p ON cp.product_id = p.id WHERE cp.id = ?", [id]);
